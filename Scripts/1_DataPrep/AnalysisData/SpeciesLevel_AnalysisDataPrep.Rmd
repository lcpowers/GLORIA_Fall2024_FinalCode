---
title: "Compile dataset for analyses"
...
veg <- read_csv("./InputData/IntermediateData/all_1x1veg_w0s_final.csv") %>% 
...
# Range data
ranges = read_csv("./InputData/IntermediateData/species_range_summary.csv")
ranges$sciname <- str_to_sentence(ranges$sciname)
ranges = ranges[ranges$n_obs>1,]
ranges = select(ranges,-c(n_obs,n_obs1090))

# water balance data
wbdata = read_csv("./InputData/IntermediateData/dailyWBdata.csv")
...
pec_clim <- read_csv("./InputData/climate/NPS_IMD_GLORIA_SoilTemperature_2288176_DataPackage_UPDATED_Pecos/NPS_IMD_GLORIA_SoilTemperature_2288176_Raw-dataset.csv") %>% 
...
traits <- read_csv("./InputData/speciesTraits/allparks_speclist_20220124 traits.csv") 
siteinfo <- read_csv("./InputData/byClaire/all_1x1veg_no0s_2022-11-01.csv") %>% 
  select(park,summit,aspect=cat.aspect,slope) %>% unique() %>% merge(.,siteinfo)

all_parks <- rbind(gnp_Ts,gsd_Ts,rmn_Ts,ynp_Ts,pec_Ts) %>% 
  filter(!str_detect(sciname," sp$")) %>% 
  merge(.,traits,by="sciname") %>% 
  merge(.,siteinfo,by=c("summit","aspect")) %>% 
  merge(.,sites.spp,by=c("sciname","summit"),all.x=T) %>% 
  merge(.,ranges,by="sciname",all.x=T) %>%
  mutate(changecover=(endcover-startcover),
         propchangecover=changecover/(startcover+0.1)) %>%
  select(park,summit,aspect,slope,sitelat,siteelev,site.probD,site.s.probD,
...
# Bringing in sas abundance
```{r SASabun}

# The file name that is read in should be updated if that script (SASveg.R) is re-run
abun <- read_csv("./InputData/byClaire/SASabundance-2022-11-07.csv") %>% 
  group_by(sciname,park,summit,year,plot,aspect) %>% 
  dplyr::summarize(abundance=sum(abundance,na.rm=T),.groups = "drop") %>% 
  # filter(abundance>0) %>% 
  arrange(year)

summits <- unique(abun$summit)

abun_wide = NULL

for(s in summits){
  
  df = filter(abun, summit==s)
  ...
  for(i in 1:(length(years)-1)){
    ...
    tmp = filter(df,year==syr|year==eyr) %>% 
      pivot_wider(id_cols = c(park,summit,aspect,sciname),
                  names_from = c(year,plot),
                  values_from = abundance,
                  names_sep = ".",
                  values_fill = 0) %>% 
      mutate(transition=paste0(s,i))
    
    colnames(tmp)[colnames(tmp)==paste0(syr,".upper")]="start.upper"
    colnames(tmp)[colnames(tmp)==paste0(syr,".lower")]="start.lower"
    colnames(tmp)[colnames(tmp)==paste0(eyr,".upper")]="end.upper"
    colnames(tmp)[colnames(tmp)==paste0(eyr,".lower")]="end.lower"
    
    abun_wide = rbind(abun_wide,tmp)
    rm(tmp)
  }
}

all_parks_h20yr_gs_abun = merge(all_parks_h20yr_gs,abun_wide,by=c("park","summit","aspect","transition","sciname"),all.x=T) %>% 
  mutate(startcover0=ifelse(startcover==0,1,0),
         endcover0=ifelse(endcover==0,1,0),
         allcover0=ifelse(startcover+endcover==0,1,0),
         expanded=ifelse(startcover<endcover,1,0),
         contracted=ifelse(startcover>endcover,1,0),
         nochange=ifelse(startcover==endcover,1,0),
         no0 = ifelse(startcover0==0&endcover0==0,1,0))
...
climcols = select(all_parks_h20yr_gs_abun,contains(c(".tmean",".snow",".gdd",".ppt",".pet",".aet"))) %>% 
  names()

pcadf = all_parks_h20yr_gs_abun %>% 
...
ggsave("Results/PCAplot.png",dpi=300,h=8,w=10)
...
all_parks_h20yr_gs_abun_pca = merge(all_parks_h20yr_gs_abun,pcdat,by=c("park","summit","aspect","transition"),all.x=T)

write_csv(all_parks_h20yr_gs_abun_pca,paste0("./AnalysisData/analysisdata_final.csv"))
```


```{r}

veg_n0s <- veg %>% filter(cover>0)
table(veg_n0s$perennating)

full_hist = ggplot()+
  geom_histogram(data = veg_n0s,aes(x=cover),bins=50,color="black",fill="grey90")+
  theme_bw(base_size=14)+
  scale_x_continuous(limits = c(0.0000001,50),expand=c(0.001,0))+
  scale_y_continuous(expand=c(0.01,0),limits=c(0,950),breaks=seq(0,800,200))+
  labs(x="% cover estimate",y="Count",title="A. Histogram of % cover estimates >0% used in analysis data")+
  geom_label(aes(label="Total count = 4,401",x=45,y=920),size=5)+
...
zoom_hist = ggplot()+
  ...
  theme_bw(base_size=14)+
  scale_x_continuous(limits=c(0.000001,5),expand=c(0.001,0))+
  scale_y_continuous(expand=c(0.01,0),limits=c(0,800),breaks=seq(0,600,200))+
  labs(x="% cover estimate",y="Count",title="B. Histogram of % cover estimates >0% and <5% used in analysis data")+
  geom_label(aes(label="Total count = 4,077",x=4.5,y=770),size=5)+
  theme(plot.title = element_text(face="bold"))
zoom_hist
cowplot::plot_grid(full_hist,zoom_hist,ncol=1)
ggsave("Results/Histogram of cover values.png",h=10,w=10)
```
