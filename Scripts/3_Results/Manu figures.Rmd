...
plantcover.us =read_csv('Data/AnalysisData/vascplantcover_final.csv') %>%
...
sppcover.orig = read_csv('Data/AnalysisData/specieslevel_analysisdata_final.csv') %>% 
  mutate(park2 = case_when(park=="GNP"~"GLAC",
                           park=="YNP"~"YELL",
                           park=="RMN"~"ROMO",
                           park=="GSD"~"GRSD",
...
climVar.mod.str = "endcover ~ startcover*(latmean1090 + I(latmean1090^2) + min.gdd + min.gs.ppt + min.wy.ppt + sitelat) + habit*(max.tmean + min.gdd + sitelat) + perenneating*max.tmean + (1 | transition) + (max.tmean + min.wy.ppt + min.gs.ppt + min.gdd | sciname)"

# Refit with REML
climVar.mod.s = mod1 <- lmer(formula = climVar.mod.str, na.action = "na.fail", data = dats.scaled, REML = T)
