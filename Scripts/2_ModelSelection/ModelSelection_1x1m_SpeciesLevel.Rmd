...
# clust <- try(makeCluster(getOption("cl.cores", 35), type = clusterType))
...
top_mod <- read_csv("Results/top_dredge_results.csv") %>% 
  filter(delta==0) %>% 
  select_if(~ !any(is.na(.)))
...
climVar_param_grid <- expand_grid(tmean.grp,snow.gdd.grp,ppt.grp,cwd.grp) %>%
...
results_list <- foreach::foreach(i = 1:nrow(climVar_param_grid), .packages = c("tidyverse","data.table","lme4","performance") )%dopar%{
  
  row.i <- climVar_param_grid[i,]
  
  tmean.i <- row.i$tmean.grp
  cwd.i <- row.i$cwd.grp
  ppt.i <- row.i$ppt.grp
  snow.gdd.i <- row.i$snow.gdd.grp
  ...
  out_df = data.table(tmean.var = tmean.i,
...
stopCluster(cl)
#clim.mod.AICs <- rbindlist(results_list)

#clim.mod.AICs$delta = clim.mod.AICs$AICc - min(clim.mod.AICs$AICc)
#clim.mod.AICs <- clim.mod.AICs %>% arrange(delta)
...
best.explClimVar.RE.mod.dredge = dredge(best.explClimVar.RE.mod.ML,
                        beta = c("none"),
                        fixed = c('startcover','min.gs.ppt','min.gdd',"min.wy.ppt","max.tmean","sitelat"),
                        trace=2,
                        subset = dc(latmean1090,I(latmean1090^2)),
                        cluster = clust)
...
best.explClimVar.mod = get.models(explClimVar.RE.mod.dredge.fltr,subset = 1)[[1]]
best.explClimVar.mod@call
summary(best.explClimVar.mod)

# Choosing the second model because it's slightly simpler and a very similar AICc value (one less interaction term)
best.climVar.mod <- get.models(best.explClimVar.RE.mod.dredge, subset = 2)[[1]]
best.climVar.mod@call
```

lmer(formula = endcover ~ habit + latmean1090 + I(latmean1090^2) + 
    perenneating + max.tmean + min.gdd + min.gs.ppt + min.wy.ppt + 
    sitelat + startcover + (1 | transition) + (max.tmean + min.wy.ppt + 
    min.gs.ppt + min.gdd | sciname) + habit:max.tmean + habit:min.gdd + 
    habit:sitelat + latmean1090:startcover + I(latmean1090^2):startcover + 
    perenneating:max.tmean + min.gdd:startcover + startcover:min.gs.ppt + 
    startcover:min.wy.ppt + sitelat:startcover, data = dats.scaled, 
